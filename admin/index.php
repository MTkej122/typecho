  ...
  <div class="col-xl-3 col-md-6">
	<div class="card bg-gradient-info card-img-holder text-white">
	  <div class="card-body">
		...
		<a href="<?php $options->adminUrl('manage-comments.php'); ?>" class="text-white mr-2"><i class="mdi mdi-basket"></i><?php _e('全部评论'); ?></a>
        <a  href="<?php $options->adminUrl('options-discussion.php'); ?>" class="text-white mr-2 "><i class="mdi mdi-book"></i><?php _e('评论设置'); ?></a>			
	  </div>
	</div>
  </div>
  <div class="col-xl-3 col-md-6">
  ...
  <div class="col-xl-3 col-md-6">
	<div class="card bg-gradient-warning card-img-holder text-white">
	  <div class="card-body">
		...
		<h2 class="mb-5"><?php switch ($user->group) {
                                    case 'administrator':
                                        _e('管理员');
                                        break;
                                    case 'editor':
	...
	<a href="<?php $options->adminUrl('manage-users.php'); ?>" class="text-white"><i class="mdi mdi-settings"></i><?php _e('用户管理'); ?></a>
	<!--data-toggle="modal" data-target="#modal-default"-->
    <a href="<?php $options->adminUrl('profile.php'); ?>" class="text-white mr-2 "><i class="mdi mdi-information"></i><?php _e('个人设置'); ?></a>
	  </div>
	</div>
  </div>
   <div class="col-md-4">
			...
			<?php while($comments->next()): ?>
			  <tr>
				...
				<div class="d-flex w-100 align-items-center">
                    <img src="<?php $email =$comments->mail;
                    if($email){if(strpos($email,'@qq.com') !==false){$email=str_replace('@qq.com','',$email);echo '//q1.qlogo.cn/g?b=qq&nk='.$email.'&s=100';}
                    else{echo '//sdn.geekzu.org/avatar/".$d."?s=220&d=identicon';}}
                    else{$email= md5($email);echo Typecho_Common::gravatarUrl($comments->mail, 220, 'X', 'mm', $request->isSecure());} ?>" alt="Image placeholder" class="avatar rounded-circle" style="width:35px;height:35px;margin-right:5px;">
                    <?php $comments->author(true); ?>
                </div>
                <a href="<?php $comments->permalink(); ?>" class="title"></a>
                <br />
